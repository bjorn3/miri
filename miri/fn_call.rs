            ...
            "sysconf" => {
                ...
                for &(path, path_value) in paths {
                    if let Ok(instance) = self.resolve_path(path) {
                        ...
                        let value = match self.tcx.interpret_interner.get_cached(instance.def_id()) {
                            Some(ptr) => {
                                Value::ByRef(MemoryPointer::new(ptr, 0).into(), name_align),
                            }
                            None => {
                                eval_body(self.tcx.tcx, cid, ty::ParamEnv::reveal_all())
                                    .ok_or_else(||EvalErrorKind::MachineError("<already reported>".to_string()).into())?.0
                            },
                        };
